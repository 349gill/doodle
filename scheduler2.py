...
class MultilevelQueue:
    ...
    def execute_tasks(self) -> Tuple[List[Task], List[Task]]:
        """Execute all tasks and return tuple of (completed_tasks, failed_tasks)."""
        while True:
            batch_tasks = self.get_next_task_batch()
            if not batch_tasks:
                break

            # Sort batch tasks by priority (highest priority first) and then by due date
            ...
            for task in batch_tasks:
                if task.can_complete_before_due(self.current_time):
                    ...
                    # Add task to failed list if it can't be completed on time
...
if __name__ == "__main__":
    ...
    completed_tasks, failed_tasks = scheduler.execute_tasks()
    ...
    if failed_tasks:
        print("\nFailed Tasks (Could not complete before deadline):")
